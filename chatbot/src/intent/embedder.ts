...
/**
 * Runs Sentence-BERT (all-MiniLM-L6-v2) fully in-process via @xenova/transformers
 * (an ONNX Runtime port of the model — no Python, no external AI service, no API key).
 *
 * The ONNX weights (~90MB, quantized) are fetched ONCE from the Hugging Face
 * model hub on first run and cached under .transformers-cache/. Every request
 * after that — including every /chat call at runtime — runs entirely offline
 * against the local cache. See README.md "Fully offline / air-gapped setup"
 * for how to pre-seed the cache and disable remote fetching entirely.
 *
 * Used identically by:
