/**
 * Ninth dataset pass -- fixes a real collision found while verifying the
 * date-parsing feature: "who is absent on 29 july" / "who is absent on 21
 * july" (no "in my class"/"in my section") misclassified as
 * admin_students_out_now (a completely different intent -- students
 * currently out on a hostel pass right now, not classroom attendance on a
 * specific date). With "in my class" added it already correctly resolved
 * to faculty_class_attendance -- the gap was specifically the bare "who is
 * absent on [date]" shape with no class-scoping words in it.
 *
 * Usage: npx tsx scripts/rebuild-dataset-v9.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  faculty_class_attendance: [
    'who is absent on 21 july',
    'who was absent on 29 july',
    'who is absent on 13 august',
    'who was present on 21 july',
  ],
};
