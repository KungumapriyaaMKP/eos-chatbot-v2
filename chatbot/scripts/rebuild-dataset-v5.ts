/**
 * Fifth dataset pass — fixes a real, live-reported bug: "what is my
 * salary" (and close variants) misclassified as get_fees (student/admin/
 * parent-only) instead of faculty_payslip (faculty-only), so a real
 * faculty user asking about their own salary got a permission-denied
 * reply. Root cause confirmed directly: every one of faculty_payslip's 30
 * training examples paired "salary" with "slip"/"statement"
 * ("salary slip", "pay statement") — none represented the extremely
 * common bare phrasing "what is my salary" / "my salary details" with no
 * "slip" attached, so nothing anchored that exact shape to faculty_payslip
 * over the financially-adjacent get_fees.
 *
 * Confirmed live before this fix: "what is my salary" -> get_fees
 * (confidence 0.721 — just above the 0.72 LLM-rerank gate, so even the
 * reranker safety net never got a chance to catch it).
 *
 * IMPORTANT INTEGRITY NOTE: fresh wording, not copied from the live bug
 * report verbatim where avoidable — though "what is my salary" itself is
 * the exact reported phrase and is included deliberately, since it's the
 * literal bug this pass exists to fix.
 *
 * Usage: npx tsx scripts/rebuild-dataset-v5.ts
 */
