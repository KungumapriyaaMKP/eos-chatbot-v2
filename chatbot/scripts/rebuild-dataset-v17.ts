/**
 * Seventeenth dataset pass -- small follow-up to v16. "top scorer in my
 * class for physics" (with a real subject named) was still losing to
 * get_marks even after v16's generic "top scorer in my class for a
 * subject" anchor -- the subject-specific phrasing apparently needed its
 * own anchor rather than relying on the generic form to generalize.
 * section-performance.service.ts already has real handler support for
 * naming the top scorer (see TOP_SCORER_PATTERN), so this is worth
 * closing properly rather than leaving it to fall to get_marks (which
 * still answers on-topic, just doesn't name the class topper).
 *
 * Usage: npx tsx scripts/rebuild-dataset-v17.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  section_performance: [
    'top scorer in my class for physics',
    'who is the top scorer in my class for dbms',
    'highest scorer in my section for a subject',
    'who topped my class in the last internal',
  ],
  admin_list_faculty: [
    'list faculty in ece department',
  ],
};
