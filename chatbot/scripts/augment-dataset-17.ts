/**
 * Seventeenth dataset augmentation pass — light anchoring for the new
 * collisions introduced by augment-dataset-16.ts's 9 new intents (all
 * same-role pairs, both sides already give a real answer, so lower
 * priority than the RBAC-risk collisions fixed earlier this session —
 * still worth a quick pass since these are common, real phrasings).
 *
 * Usage: npx tsx scripts/augment-dataset-17.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  get_wallet_balance: ['how much money is in my wallet right now', 'current wallet balance'],
  wallet_recharge: ['add funds to my wallet', 'i need to top up my campus card balance'],
  get_active_surveys: ['is there a survey open for me', 'any pending evaluation forms'],
  submit_feedback_form: ['take me to the feedback form', 'i need to fill out an evaluation'],
  get_my_projects: ['who is my project mentor', 'my project guide name'],
  alumni_network_search: ['alumni working in a particular company', 'find alumni by company name'],
};
