/**
 * Adds a brand-new intent, get_upcoming_events -- a real gap found live:
 * calendar_event_type_enum has TWO values ('holiday' and 'event'), but
 * only the 'holiday' half was ever read by anything (get_holidays).
 * "upcoming events" / "events happening today" is a completely
 * legitimate, real-data-backed question that had nowhere to go -- in one
 * live case it fell through to faculty_class_attendance (an RBAC-denied
 * intent for a student) purely because both mention "today".
 * getUpcomingEvents (holidays.service.ts) was already written and wired
 * into the registry; this just adds the matching training data.
 *
 * Same safe pipeline as add-institution-performance-intent.ts /
 * add-assignments-intent.ts: merge onto the current intents.json, rebuild
 * the .docx, round-trip-verify before trusting it.
 *
 * Usage: npx tsx scripts/add-upcoming-events-intent.ts
 */
...
const NEW_INTENT: IntentDefinition = {
  ...
  roles: ['student', 'faculty', 'admin', 'parent', 'hod'],
  description:
    "Upcoming campus events (seminars, fests, deadlines) from the academic calendar -- event_type='event', distinct from get_holidays (event_type='holiday', which also implies no classes that day).",
  examples: [
    'upcoming events',
    ...
    'any events planned for this semester',
...
async function main() {
  ...
  // Insert right after get_holidays so it stays grouped with the other calendar intent.
  const insertAfter = current.intents.findIndex((i) => i.name === 'get_holidays');
