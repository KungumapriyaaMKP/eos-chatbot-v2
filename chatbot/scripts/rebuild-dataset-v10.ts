/**
 * Tenth dataset pass -- fixes 6 real abbreviation/short-form collisions
 * found during a systematic sweep (testing common short forms across many
 * domains). All verified reproducible (ran each twice, identical wrong
 * result both times) before writing this fix:
 *
 * - "hod contact" -> thanks/get_hall_ticket (should be oos_faculty_contact)
 * - "my dob" -> greeting (should be get_profile -- date_of_birth is a real
 *   field the profile handler already shows for students)
 * - "my sec" -> thanks/get_hall_ticket (should be get_profile -- section is
 *   a real profile field)
 * - "exam sched" -> get_marks (should be get_exam_schedule -- "sched"
 *   abbreviation for "schedule" wasn't anchored anywhere)
 * - "result pub status" -> null, below threshold (should be
 *   get_result_publication_status -- "pub" abbreviation for "publication")
 * - "hostel outing req" -> get_hostel_room (should be get_outing_status --
 *   an outing REQUEST is not room allocation)
 *
 * Usage: npx tsx scripts/rebuild-dataset-v10.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  ...
  get_result_publication_status: [
    ...
    'has result pub happened',
  ],
  get_outing_status: [
    'hostel outing req',
    'my outing req status',
    'outing req approved?',
  ],
};
