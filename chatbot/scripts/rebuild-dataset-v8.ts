/**
 * Eighth dataset pass — fixes two real, live-reported bugs on the same
 * screenshot:
 *
 * 1. "who are present today" (no "in my class") misclassified as
 *    admin_visitor_log instead of faculty_class_attendance, causing an
 *    RBAC denial for a real faculty user. Root cause confirmed directly:
 *    faculty_class_attendance had "who was absent in my class today" but
 *    NO equivalent bare "who is/are present today" example at all (only
 *    "how many were present" — a COUNT question, not a "who" list
 *    question) — the short "who ... today" shape drifted toward
 *    admin_visitor_log's own short examples ("visitor log for today",
 *    "who visited yesterday evening").
 *
 * 2. "who are presne ttoday in my class" (heavy typo of "who are present
 *    today in my class") misclassified as get_timetable at confidence
 *    1.000. faculty_class_attendance already has typo'd examples, but all
 *    of them typo the word "attendance" itself ("atendence", "attandance")
 *    — none typo "present"/"today", so a message that garbles exactly
 *    those two words has no close neighbor to land on.
 *
 * Usage: npx tsx scripts/rebuild-dataset-v8.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  faculty_class_attendance: [
    ...
    "who's present today in my class",
    'who is present today in my class',
    'which students are present today',
    'who are presnt tody in my class',
    'who is absnt today in my class',
  ],
};
