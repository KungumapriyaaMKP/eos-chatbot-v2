/**
 * Sixth dataset pass — fixes a real, live-reported bug: "my od status?"
 * misclassified as get_bonafide_status instead of get_od_status. Root
 * cause confirmed directly: get_bonafide_status has the exact bare
 * 2-word example "bonafide status" (plus close variants — "bonafide
 * status please", "bonafide status?"), but get_od_status had NO
 * equivalent bare "od status" form — only longer phrasings ("check od
 * application status", "did the hod approve my od"). A short query
 * structurally resembles whichever intent has anchored that exact short
 * shape, regardless of the specific word inside it, so "od status" drifted
 * toward bonafide's anchored short form instead of od's own examples.
 *
 * Confirmed live: "od status" -> get_bonafide_status (0.736, ABOVE the
 * 0.72 rerank ceiling, so the reranker safety net never even got a chance
 * to fix it). "my od status ?" -> get_bonafide_status (0.591, below the
 * ceiling, but the reranker call itself TIMED OUT (8000ms) at that
 * moment, so the wrong SBERT pick stood anyway) -- both are the exact
 * scenario in the screenshot.
 *
 * Usage: npx tsx scripts/rebuild-dataset-v6.ts
 */
