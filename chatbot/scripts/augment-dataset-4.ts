/**
 * Fourth dataset augmentation pass — found via live RBAC-expansion testing:
 * bare "what subjects" / "my subjects" (no "this semester" qualifier) was
 * landing on faculty_my_classes instead of get_my_subjects, since every
 * existing get_my_subjects example includes a semester qualifier
 * ("this semester", "this sem", "sem 4", ...) and nothing bare enough to
 * anchor the short, unqualified phrasing — while faculty_my_classes has
 * "subjects assigned to me", close enough in embedding space to win.
 *
 * Usage: npx tsx scripts/augment-dataset-4.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  get_my_subjects: ['what subjects', 'my subjects', 'what are my subjects', 'show my subjects', 'subjects list'],
};
