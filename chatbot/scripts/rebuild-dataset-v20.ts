/**
 * Twentieth dataset pass -- fixes a doubly-embarrassing real bug found
 * live: "search for data structures" (bare, no "book"/"library" word) was
 * landing on get_subject_notes (0.752), not search_books -- and this
 * exact phrase is the app's OWN suggested example text (search_books'
 * clarification message literally says 'Example: "Search for Data
 * Structures"'). The bot's own suggested rephrasing didn't even work.
 *
 * Root cause: "data structures" is both a real book-catalog entry AND a
 * curriculum subject name, and search_books had no BARE "search for
 * <subject-like word>" example (only "search for operating system
 * books", always with a trailing "book(s)" word) for SBERT to anchor
 * against -- so a bare "search for X" pulled toward get_subject_notes'
 * "study material for X" territory instead.
 *
 * Usage: npx tsx scripts/rebuild-dataset-v20.ts
 */
...
const NEW_EXAMPLES: Record<string, string[]> = {
  search_books: [
    ...
    'is data structures available in the library',
